...
boolean execute(object aProcess, int maxsteps) {
	...
	readLinkageBlock: contextObject = processStackAt(linkPointer+1);
	...
	readMethodInfo: lits = sysMemPtr(basicAt(method, literalsInMethod));
	...
	while (--timeSliceCounter > 0) {
		...
		case SendMessage:
			...
			doSendMessage: arg = psb + (returnPoint - 1);
			if (isInteger(argumentsAt(0)))
